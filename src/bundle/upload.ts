...
export async function uploadBundle(appid: string, options: Options, shouldExit = true) {
  try{
    ...
    p.log.error(`Error uploading bundle: ${uploadError}`);
    
    await deleteFailedVersion(supabase, appid, versionData);
  }
}

async function deleteFailedVersion(supabase, appid, versionData) {
  try {
    // Make a request to the backend to delete the failed version
    await supabase
      .from('versions')
      .delete()
      .eq('app_id', appid)
      .eq('name', versionData.name) 
      .eq('bucket_id', versionData.bucket_id) 
      .eq('user_id', versionData.user_id)
      .single();
      
    p.log.info('Failed version deleted successfully.');
  } catch (deleteError) {
    p.log.error(`Error deleting failed version: ${deleteError}`);
  }
}
