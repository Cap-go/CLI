...
import { encryptSource, privateEncryptSource } from '../api/crypto'
import { baseKey, baseKeyPub, getLocalConfig } from '../utils'
...
export async function privateEncryptZip(zipPath: string, options: Options) {
  p.intro(`Encryption`)
  ...
  if (!existsSync(keyPath) && !privateKey) {
    p.log.warning(`Cannot find private key ${keyPath} or as keyData option`)
    if (ciDetect.isCI) {
  ...
  const zipFile = readFileSync(zipPath)
  const encodedZip = privateEncryptSource(zipFile, privateKey)
  p.log.success(`ivSessionKey: ${encodedZip.ivSessionKey}`)
  // write decodedZip in a file
  writeFileSync(`${zipPath}_encrypted.zip`, encodedZip.encryptedData)
  p.log.success(`Encrypted zip saved at ${zipPath}_encrypted.zip`)
