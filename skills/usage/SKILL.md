...
- `run device [platform]`: test the same run-on-device picker used by init onboarding without running the full onboarding. Use `npx @capgo/cli@latest run device ios --no-launch` to exercise iOS physical/simulator target selection and print the resolved `cap run` command without launching the app.
