...
Interactive prompts are disabled automatically in CI and other non-interactive environments so uploads do not block automation.
When those optional prompts appear interactively, you can choose to remember your answer on the current machine so the CLI does not ask again.
